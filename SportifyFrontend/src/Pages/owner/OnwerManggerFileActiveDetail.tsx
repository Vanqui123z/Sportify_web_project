...
const ManagerFieldActiveDetail: React.FC = () => {
  ...
  // Parse giờ từ shiftName (ví dụ: "Buổi chiều: 16H00-17H00" -> {start: "16H00", end: "17H00"})
  const parseTimeFromShift = (shiftName: string) => {
    const timeMatch = shiftName.match(/(\d{2}H\d{2})-(\d{2}H\d{2})/);
    if (timeMatch) {
      const result = {
        startTime: timeMatch[1], // "16H00"
        endTime: timeMatch[2]    // "17H00"
      };
      return result;
    }
    return null;
  ...
  const getShiftBookingStats = () => {
    ...
    bookings.forEach(booking => {
      ...
      const timeInfo = parseTimeFromShift(booking.shiftName);
      if (timeInfo) {
  ...
  const isBookingInTimeRange = (booking: BookingData, date: Date) => {
    ...
    // Parse thời gian từ shiftName
    const timeInfo = parseTimeFromShift(booking.shiftName);
    if (!timeInfo) return null;

    // Xử lý theo loại booking
    if (booking.type === 'ONCE') {
      // Booking 1 lần - check ngày cụ thể
      ...
      // Booking cố định - check trong khoảng thời gian và ngày trong tuần
      const isInDateRange = date >= bookingStart && date <= bookingEnd;

      // Xử lý dayOfWeek (2=Monday, 7=Saturday, 1=Sunday, 8=Monday tuần sau)
      let targetDayOfWeek = booking.dayOfWeek;
      if (targetDayOfWeek === 8) targetDayOfWeek = 2; // 8 = Monday

      // getDay(): 0=Sunday, 1=Monday, ..., 6=Saturday
      // Map to: 2=Monday, ..., 7=Saturday, 1=Sunday
      let currentDayOfWeek = date.getDay();
      if (currentDayOfWeek === 0) currentDayOfWeek = 1; // Sunday
      else currentDayOfWeek += 1; // Monday=2, ..., Saturday=7
