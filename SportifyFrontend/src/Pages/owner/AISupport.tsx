...
const AiSupportPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchForecasts = fetch(`${URL_BACKEND}/api/user/forecast/next-week-owner?ownerUsername=${username}`)
      .then(res => res.json());

    // Fetch field usage data
    const fetchFieldUsage = fetch(`${URL_BACKEND}/api/field-usage/active-fields/by-7daylast?ownerUsername=${username}`)
      .then(res => res.json());
