...
public class GeminiServiceImpl implements AIService {
    ...
    public String chat(String message) {
        ...
        try {
            ...
            String contactHTML = buildContactHTML(contacts);
     ...
     */
    private String buildEventHTML(List<Event> events) {
        ...
        return "<h3>⚽ Danh Sách Đội/Sự Kiện</h3>" + events.stream()
            ...
            .map(event -> String.format(
                ...
                event.getDateevent() != null ? event.getDateevent() : "Chưa xác định",
                event.getLocation() != null ? event.getLocation() : "Chưa xác định"
            ))
            .collect(Collectors.joining("\n"));
    }

    /**
     * Xây dựng thông tin liên hệ dưới dạng HTML
     */
    private String buildContactHTML(List<Contact> contacts) {
        if (contacts == null || contacts.isEmpty()) {
            return "<h3>📞 Thông Tin Liên Hệ</h3><p>Chưa có thông tin liên hệ.</p>";
        }
        
        return "<h3>📞 Thông Tin Liên Hệ</h3>" + contacts.stream()
            .limit(5)
