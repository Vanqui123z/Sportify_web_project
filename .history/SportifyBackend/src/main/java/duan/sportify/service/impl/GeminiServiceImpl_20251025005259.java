    ...
    private String buildProductHTML(List<Products> products) {
        ...
        return "<h3>📦 Danh Sách Sản Phẩm</h3>" + products.stream()
            ...
            .map(product -> String.format(
                "<div style=\"border: 1px solid #ddd; padding: 10px; margin: 5px 0; border-radius: 4px; background: #fffacd;\">" +
                "<strong>%s</strong> - %s VND<br>" +
                "%s<br>" +
                "<a href=\"http://localhost:5173/user/product-detail/%s\" style=\"color: #007bff; text-decoration: none; font-size: 12px;\">Xem sản phẩm</a>" +
                "</div>",
