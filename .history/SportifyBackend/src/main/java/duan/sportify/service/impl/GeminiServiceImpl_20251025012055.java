...
public class GeminiServiceImpl implements AIService {
    ...
    /**
     * Xây dựng danh sách sản phẩm dưới dạng HTML với hình ảnh
     */
    private String buildProductHTML(List<Products> products) {
        ...
        return "<h3>📦 Danh Sách Sản Phẩm</h3>" + products.stream()
            .limit(15)
            .map(product -> {
                String imageName = (product.getImage() != null && !product.getImage().isEmpty()) 
                    ? product.getImage() 
                    : "default.png";
                // Kiểm tra nếu là Cloudinary URL (bắt đầu với 'v' hoặc chứa '/')
                String imageUrl = (imageName.startsWith("v") || imageName.contains("/")) 
                    ? imageName  // Cloudinary URL - dùng trực tiếp
                    : "/user/images/" + imageName;  // Local image - thêm path
                return String.format(
                    "<div style=\"border: 1px solid #a5d6a7; padding: 10px; margin: 8px 0; border-radius: 6px; background: white; display: flex; gap: 10px;\">" +
                    "<img src=\"%s\" alt=\"%s\" style=\"width: 60px; height: 60px; object-fit: cover; border-radius: 4px;\" />" +
                    "<div style=\"flex: 1;\">" +
                    "<strong style=\"color: #1b5e20; display: block;\">%s</strong>" +
                    "<span style=\"color: #2e7d32; font-weight: bold; display: block;\">%s VND</span>" +
                    "<small style=\"color: #558b2f; display: block;\">%s</small>" +
                    "<a href=\"http://localhost:5173/user/product-detail/%s\" style=\"color: #2e7d32; text-decoration: none; font-weight: 500; font-size: 12px; border-bottom: 1px solid #81c784;\">Xem chi tiết →</a>" +
                    "</div>" +
                    "</div>",
                    imageUrl,
                    product.getProductname() != null ? product.getProductname() : "Sản phẩm",
                    product.getProductname(),
                    String.format("%,d", product.getPrice() != null ? product.getPrice() : 0),
                    product.getDescriptions() != null ? product.getDescriptions().substring(0, Math.min(50, product.getDescriptions().length())) + "..." : "Sản phẩm thể thao chất lượng cao",
