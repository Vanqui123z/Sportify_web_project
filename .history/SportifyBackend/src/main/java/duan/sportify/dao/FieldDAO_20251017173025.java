...
public interface FieldDAO extends JpaRepository<Field, Integer> {
	...
	@Query(value = "SELECT f.*, " +
           "6371 * 2 * ASIN(SQRT(POWER(SIN(RADIANS((:userLat - f.latitude) / 2)), 2) + " +
           "COS(RADIANS(:userLat)) * COS(RADIANS(f.latitude)) * POWER(SIN(RADIANS((:userLng - f.longitude) / 2)), 2))) AS distance " +
           "FROM field f " +
