...
public interface FieldDAO extends JpaRepository<Field, Integer> {
	...
	@Query(value = "SELECT f.*, " +
           "6371 * acos(cos(radians(:userLat)) * cos(radians(f.latitude)) * cos(radians(f.longitude) - radians(:userLng)) + sin(radians(:userLat)) * sin(radians(f.latitude))) AS distance " +
           "FROM field f " +
	...
	List<Object[]> findNearestFields(@Param("userLat") Double userLat, @Param("userLng") Double userLng, 
                               @Param("sporttypeId") String sporttypeId, @Param("limit") Integer limit);
}
