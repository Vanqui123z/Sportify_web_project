...
public interface FieldDAO extends JpaRepository<Field, Integer> {
	...
	@Query(value = "SELECT f.*, " +
           "6371 * acos(LEAST(1, cos(radians(:userLat)) * cos(radians(f.latitude)) * cos(radians(f.longitude) - radians(:userLng)) + sin(radians(:userLat)) * sin(radians(f.latitude)))) AS distance " +
           "FROM field f " +
           ...
           "HAVING distance < 50 " +  // Tìm trong bán kính 50km
