...
public interface FieldDAO extends JpaRepository<Field, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "DELETE FROM bookingdetails WHERE fieldid = :fieldId", nativeQuery = true)
	...
	@Query(value = "SELECT f.*, " +
           "ROUND(6371 * 2 * ASIN(SQRT(POWER(SIN((RADIANS(:userLat) - RADIANS(f.latitude)) / 2), 2) + COS(RADIANS(:userLat)) * COS(RADIANS(f.latitude)) * POWER(SIN((RADIANS(:userLng) - RADIANS(f.longitude)) / 2), 2))), 2) AS distance " +
           "FROM field f " +
           "WHERE (f.sporttypeid = :sporttypeId OR :sporttypeId = 'tatca') AND f.status = 1 AND f.latitude IS NOT NULL AND f.longitude IS NOT NULL " +
           "HAVING distance <= 100 " +  // Mở rộng tìm sân trong bán kính 100km
           "ORDER BY distance", nativeQuery = true)
	List<Object[]> findNearestFields(@Param("userLat") Double userLat, @Param("userLng") Double userLng, 
