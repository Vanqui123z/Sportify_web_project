...
-- 4. Đánh dấu database đã được cập nhật tọa độ
INSERT INTO flyway_schema_history_sportify (installed_rank, version, description, type, script, checksum, installed_by, execution_time, success)
VALUES (
    (SELECT MAX(installed_rank) + 1 FROM flyway_schema_history_sportify),
    '6',
    'Fix All Field Coordinates',
    'SQL',
    'V6__Fix_All_Field_Coordinates.sql',
    0,
    'ADMIN',
    100,
    1
) ON DUPLICATE KEY UPDATE installed_by = 'ADMIN';