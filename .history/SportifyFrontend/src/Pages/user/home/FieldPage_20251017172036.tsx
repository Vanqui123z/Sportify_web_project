...
export default function FieldPage() {
  ...
  useEffect(() => {
    ...
    if (latitude && longitude) {
      ...
      const validLatitude = parseFloat(latitude || "0");
      const validLongitude = Math.abs(parseFloat(longitude || "0")); // Đảm bảo longitude dương
      
      console.log('Đang gọi API với tọa độ đã kiểm tra:', validLatitude, validLongitude);
      
      fetch(`/api/sportify/field/nearest?latitude=${validLatitude}&longitude=${validLongitude}&categorySelect=${categoryParam}&limit=50`)
        .then(response => {
