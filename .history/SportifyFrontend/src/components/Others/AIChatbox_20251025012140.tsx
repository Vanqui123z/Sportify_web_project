  ...
  const ask = async (msg: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessages((msgs) => [...msgs, { role: "bot", text: `😅 Lỗi từ server (${res.status}). Vui lòng thử lại sau.` }]);
        setIsLoading(false);
        return;
      }
      ...
      if (data && data.reply) {
        ...
        console.warn("Response không chứa reply:", data);
        setMessages((msgs) => [...msgs, { role: "bot", text: "Xin lỗi, hiện chưa nhận được phản hồi." }]);
      }
    } catch (error) {
      console.error("Chat error:", error);
      setMessages((msgs) => [...msgs, { role: "bot", text: "❌ Lỗi kết nối đến AI. Kiểm tra console để xem chi tiết." }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="ai-chat-body" ref={bodyRef}>
          {messages.length === 0 ? (
            <div style={{ textAlign: "center", marginTop: "20px" }}>
              ...
              <div style={{ color: "#2e7d32", fontWeight: "600", marginBottom: "16px" }}>
                Xin chào! Tôi là Sportify AI
              </div>
              <div style={{ color: "#558b2f", fontSize: "14px", marginBottom: "20px" }}>
