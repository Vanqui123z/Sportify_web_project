  ...
  const loadChatHistoryFromDatabase = async () => {
    try {
      ...
      if (data.status === "success" && data.data && data.data.length > 0) {
        ...
        // Merge with localStorage messages (prefer localStorage if exists)
        const localMessages = JSON.parse(localStorage.getItem("aichatbox_messages") || "[]");
    ...
    if (message.text) {
      ...
      if (message.text.includes('<')) {
        return (
          <div 
            key={index} 
            className="ai-msg ai-bot ai-html-content"
