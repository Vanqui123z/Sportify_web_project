    ...
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Chuyển hướng với tọa độ - chuyển thành gọi API trực tiếp để tránh lỗi
          // Sửa đường dẫn để khớp với API backend đúng
          navigate(`/sportify/field/nearest?latitude=${lat}&longitude=${lng}&timestamp=${new Date().getTime()}`);
        },
        (error) => {
          clearTimeout(timeoutId);
