  ...
  useEffect(() => {
    if (userLocation && fields.length > 0) {
      const fieldsWithDistance = fields.map((field) => {
        // For testing purposes - should be replaced with actual coordinates from your database
        const fieldLat = field.location?.latitude;
        const fieldLng = field.location?.longitude;
