...
export async function fetchProductDetail(productid: any) {
  const res = await fetch(`http://localhost:8081/api/sportify/product-single/${productid}`);
  if (!res.ok) throw new Error('Failed to fetch product detail');
  return res.json();
}

export async function addProductToCart(productid: any, quantity: any) {
  const res = await fetch(`http://localhost:8081/api/user/cart/add/${productid}?quantity=${quantity}`, {
    method: 'POST',
    credentials: 'include',
